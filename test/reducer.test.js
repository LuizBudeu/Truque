...
describe("special-card movement", () => {
    ...
    test("pushAmount is required with K and rejected without it", () => {
        ...
        assert.equal(isLegalAction(withK, 0, { type: "CHOOSE_MOVE", player: 0, selfOffset: 0 }).legal, false);
        const withoutK = makeState({
            phase: "WINNER_MOVE",
            lastResolution: { winner: 0, winnerMoveRange: 2, loserEffect: { type: "RETREAT", amount: 1 } },
        });
        assert.equal(isLegalAction(withoutK, 0, { type: "CHOOSE_MOVE", player: 0, selfOffset: 0, pushAmount: 2 }).legal, false);
    });
});
...
describe("danger-zone open play (Rulebook 2.9; §10 Q8)", () => {
    ...
    test("both endangered → normal simultaneous play (Rulebook 2.9)", () => {
        ...
        assert.equal(isLegalAction(s0, 0, { type: "PLAY_CARD", player: 0, card: C(2, "hearts") }).legal, true);
    });
});
...
describe("illegal actions and immutability", () => {
    test("phase gating and basic rejections", () => {
        ...
        assert.equal(isLegalAction(committed, 0, { type: "PLAY_CARD", player: 0, card: C(5, "spades") }).legal, false);
    });
