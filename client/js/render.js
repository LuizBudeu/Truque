 ...
 *
 * The same game screen serves hotseat and online: `model.online` switches
 * player labels ("You" vs "Player N"), enables the waiting states for the
...
function gameHTML(model) {
  ...
  return `
    <div class="screen game">
      ${hudHTML(view, { concede: !over, concedeArmed: ui.concedeArmed })}
...
function gameOverHTML(model) {
  ...
  if (view.winner === 'draw') {
    ...
    if (conceded) {
      message =
        view.winner === seat
          ? 'Your opponent conceded — you win the game!'
          : 'You conceded — your opponent wins the game.';
    } else {
      message = view.winner === seat ? 'You win the game!' : 'You lose — your opponent wins the game.';
    }
  } else {
    message = conceded
      ? `Player ${view.concededBy + 1} conceded — Player ${view.winner + 1} wins the game!`
