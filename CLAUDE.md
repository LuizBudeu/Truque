...
Track progress in `docs/PLANNING.md` §9. Each phase must be verifiably done before the next starts. Phases 0–7 are complete. Rules engine fully implemented and tested; hotseat UI plays complete games in one tab; online play works — rooms with 4-letter join codes, authoritative `ws` server, per-player views, and refresh/drop rejoin via `playerToken` in `sessionStorage`. Phase 5 (polish) added WebAudio-synthesized sound effects (`client/js/sound.js`, a browser-only leaf module like `net.js`; cues fire from the animation plan in `main.js`, HUD mute toggle persisted in `localStorage`), an online rematch (both seats vote via `REQUEST_REMATCH`/`REMATCH_STATE`; `Room.requestRematch` replays with a fresh seed), a copy-room-link lobby button with one-click `?room=CODE` join, a round-history log (a public `history` array on the state → view → `client/js/ui/log.js`), and a mobile responsive pass (600px breakpoints; the board shrinks to fit a phone and scrolls inside itself below ~340px).

**Ruleset variants (Phase 7).** The game supports selectable rulesets: `legacy` (the finalized rules, frozen) and `v2` (board shrinks one space each edge per play-deck reshuffle; ♥ magic becomes an opponent-dependent scaled-triangle modifier instead of a flat 0). A ruleset is a **policy object of pure seam functions + data** in `shared/rulesets/` (`legacy` is the base; `v2` spreads it and overrides only `suitModifier`/`onReshuffle`, delegating the rest) — the reducer calls the seams, so there are **no `ruleset === …` branches in the engine**. The chosen id lives on `state.ruleset` (absent → `legacy`) and travels through `view.ruleset`; the live board extent is `state.bounds` (`{min,max}`), so `BOARD_SIZE`/`DANGER_SPACES` are derived via `dangerSpaces(bounds)`/`boardSize(bounds)`, never assumed to be 0–11. The creator picks the ruleset at room creation (`CREATE_ROOM {ruleset}`, validated server-side, preserved across rematch/rejoin, persisted in `localStorage`); joiners inherit it. To add a ruleset: add a policy object and register it in `shared/rulesets/index.js`, and **add seams lazily** — one per rule that actually needs one. Decisions in PLANNING.md §10 (Q16–Q19).
